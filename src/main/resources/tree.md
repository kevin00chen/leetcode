...
### 437 [EASY][二叉树和为指定值的全部路径](../java/com/ckm/tree/easy/Solution437.java)
问题描述：
...
　　序列化比较相对简单，深度遍历二叉树，遇到空节点用`null`表示，需要借助队列。另外需要注意的是去除字符串最后的`,`以及`null,`。

　　反序列化也需要借助队列，取第一个元素，如果是数字就生成root节点，然后依次取下一个元素生成左子节点，以及右子节点。
生成的节点存入队列，继续遍历字符串中的有效元素
...
### 99 [HARD][BST结构维护-互换两个元素](../java/com/ckm/tree/hard/Solution99.java)
问题描述：

　　在给定的二叉搜索树(BST)中有两个元素位置发生了互换，找到这个变换，然后将这两个元素调整到正确的位置，尽量少的使用额外空间。
...
### 986 [HARD][二叉树节点全覆盖](../java/com/ckm/tree/hard/Solution986.java)
问题描述：
...
　　给定一个非空二叉树，找到其中最大的路径和。这里的路径可以是从任意节点开始到任意节点结束，而不必非得经过root节点。但是路径中最少有一个节点。
...
　　递归。对于一个节点，分别计算左右子树中最大路径和`pml, pmr`，然后计算从左右子节点出发的最大路径和`rml, rmr`。
那么这棵树的最大路径和为`max(pml, pmr, rml + rmr + root.val)`，并且经过根节点的最大路径和为`max(root.val + rml, root.val  + rmr)`。

### 补充题目[][](../java/com/ckm/tree/add/SolutionTreeAdd1.java)
问题描述：
　　给定一棵二叉树，找出从根节点出发的路径中，和最大的一条。
